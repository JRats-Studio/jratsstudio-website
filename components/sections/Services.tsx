  ...
  return (
    <div ref={container} className="h-screen flex items-center justify-center sticky top-0">
      <motion.div
        ...
        className="relative flex flex-col h-auto min-h-[400px] md:h-[480px] w-full max-w-[90vw] sm:max-w-[85vw] md:max-w-[1200px] rounded-2xl p-5 sm:p-7 md:p-9 origin-top bg-[#0a0a0a] border border-white/ overflow-hidden"
      >
        {/* Subtle overlay for depth – optional */}
        <div className="absolute inset-0 bg-black/20 rounded-2xl pointer-events-none" />

        <div className="relative z-10 h-full flex flex-col md:flex-row gap-5 md:gap-9 items-center justify-center p-2">
          {/* Left Side: Icon & Title */}
          <div className="flex-1 flex flex-col items-center md:items-start space-y-4 text-center md:text-left">
            <motion.div
              ...
              whileInView={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5 }}
              className="w-13 h-13 rounded-xl bg-[#08cb00]/20 border border-[#08cb00]/40 flex items-center justify-center"
            >
              <service.icon className="w-7 h-7 text-[#08cb00]" />
            </motion.div>
            ...
            <div>
              <motion.h4
                ...
                whileInView={{ opacity: 1, x: 0 }}
                transition={{ duration: 0.5, delay: 0.1 }}
                className="font-mono text-[#08cb00] text-md tracking-[0.2em] uppercase mb-2"
              >
                {service.subtitle}
              </motion.h4>
              <motion.h3
                ...
                whileInView={{ opacity: 1, scale: 1 }}
                transition={{ duration: 0.5, delay: 0.2 }}
                className="font-heading text-2.5xl sm:text-3.5xl md:text-4.5xl lg:text-7xl text-white uppercase leading-[0.9] tracking-tighter"
              >
                {service.title}
              </motion.h3>
            </div>
          ...
          {/* Right Side: Description & Details */}
          <div className="flex-1 flex flex-col items-center md:items-start space-y-4 bg-[#0A0A0A]/5 backdrop-blur-sm p-5 rounded-xl border border-white/ w-full">
            <motion.p
              initial={{ opacity: 0 }}
              whileInView={{ opacity: 1 }}
              transition={{ duration: 0.5, delay: 0.3 }}
              className="font-mono text-gray-300 text-sm sm:text-base lg:text-lg leading-relaxed text-center md:text-left"
            >
              {service.description}
            </motion.p>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-3 border-t border-white/ w-full">
              {service.capabilities.map((cap: string, i: number) => (
                <motion.div
                  key={i}
                  initial={{ opacity: 0, x: 10 }}
                  whileInView={{ opacity: 1, x: 0 }}
                  transition={{ duration: 0.3, delay: 0.4 + (i * 0.1) }}
                  className="flex items-center justify-center md:justify-start gap-2 text-md sm:text-md text-gray-200 font-mono"
                >
                  <Check className="w-4 h-4 text-[#08cb00] flex-shrink-0" />
                  <span>{cap}</span>
                </motion.div>
              ))}
            </div>
...
export const Services = () => {
  ...
  return (
    <section ref={container} className="relative bg-black pt-18 pb-18">
      {/* PixelBlast Background */}
      <div className="absolute inset-0 z-0">
        {showEffects && (
          <PixelBlast
      ...
      <div className="relative z-10 mb-14 sm:mb-18 md:mb-22 px-4 sm:px-6 md:px-8 max-w-[95vw] sm:max-w-[90vw] md:max-w-[1400px] mx-auto flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h2 className="font-heading text-6xl sm:text-4.5xl md:text-5.5xl text-white uppercase tracking-tight">
            The <span className="text-gradient-premium">Arsenal</span>
          </h2>
          <div className="w-18 h-1 bg-[#08cb00] mt-3" />
        </div>
        <p className="font-mono text-gray-400 text-base max-w-sm text-right hidden md:block">
          / SYSTEMS DESIGNED FOR<br />
          <span className="text-[#08cb00]">TOTAL VELOCITY</span>
        </p>
      </div>
      ...
      <div className="relative z-10">
        {services.map((service, index) => {
          const targetScale = 1 - ((services.length - index) * 0.045);
          return <ServiceCard key={index} index={index} service={service} range={[index * .25, 1]} targetScale={targetScale} progress={scrollYProgress} />
        })}
      </div>
    </section>
